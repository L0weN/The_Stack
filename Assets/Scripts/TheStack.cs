...
public class TheStack : MonoBehaviour
{
    ...
    private const float STACK_BOUND_GAIN = 0.1f;
    ...
    bool PlaceTile()
    {
        ...
        if (isMovingX)
        {
            ...
            if (Mathf.Abs(deltaX) > ERROR_MARGIN)
            {
                ...
                CreateRubble(new Vector3((t.position.x > 0) ? t.position.x + (t.localScale.x / 2f) : t.position.x - (t.localScale.x / 2f), t.position.y, t.position.z),
        ...
        else
        {
            ...
            if (Mathf.Abs(deltaZ) > ERROR_MARGIN)
            {
                ...
                t.localScale = new Vector3(stackBounds.x, 1f, stackBounds.y);
                CreateRubble(new Vector3(t.position.x, t.position.y, (t.position.z > 0 ? t.position.z + (t.localScale.z / 2f) : t.position.z - (t.localScale.z / 2f))),
